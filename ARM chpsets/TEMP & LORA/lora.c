...
static unsigned char dht11_data[8], data_string_1[8], data_string_2[8], display[30] = "Humidity and Temperature";
...
void Init_Uart0(void)
{
	...
	LPC_IOCON->P0_2 |= 1;//TXD0
	LPC_IOCON->P0_3 |= 1;//RXD0
	
	//select clock source and frequency
	//8 bits, no parity, 1 stop bit
	//set DLAB=1 to access baud rate
	LPC_UART0->LCR = 0x83;
	
	//derive baud rate from uart clock 
	//DLM:DLL=PCLK
	LPC_UART0->DLL = 16;//lora baud rate
	LPC_UART0->DLM = 0;
	LPC_UART0->LCR = 0x03;
}
